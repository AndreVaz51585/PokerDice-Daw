...
class MatchServiceImpl(
    ...
    private fun Map<Int, Hand>.toRoundHandView(): Map<Int, MatchEvent.RoundSummary.RoundHandView> =
        mapValues { (_, hand) ->
    ...
    override fun getMatch(id: Int): Either<MatchServiceError, Match> =
        trxManager.run {
    ...
    override fun getBankedMatch(matchId: Int): BankedMatch? = matchManager.get(matchId)?.snapshot()
    ...
    override fun registerBankedMatchFromDb(matchId: Int): Either<MatchServiceError, Boolean> {
        ...
        val wallets =
            matchPlayers.associate { p ->
                val wallet =
                    when (val walletResult = walletService.getWallet(p.userId)) {
