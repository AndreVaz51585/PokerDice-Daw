...
dependencies {
    ...
    implementation(project(":poker-dice-backend:domain"))
    implementation(project(":poker-dice-backend:repo")) // se as interfaces Repository*/Transaction* estiverem aqui
    implementation(project(":poker-dice-backend:http"))
    implementation(project(":poker-dice-backend:service"))
...
val dockerComposePath = composeFileDir.file("poker-dice-backend/SQL/docker-compose.yml").toString()
val dockerExe =
    when (
