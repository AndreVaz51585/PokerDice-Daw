...
class BankedMatchEngineTests {
    ...
    private fun joinTwoPlayers(state: BankedMatch): BankedMatch {
        val s1 = BankedMatchEngine.apply(state, Command.Join(1))
        val s2 = BankedMatchEngine.apply(s1, Command.Join(2))
        return s2
    }
    ...
    private fun joinThreePlayers(state: BankedMatch): BankedMatch {
        val s1 = BankedMatchEngine.apply(state, Command.Join(1))
        val s2 = BankedMatchEngine.apply(s1, Command.Join(2))
        val s3 = BankedMatchEngine.apply(s2, Command.Join(3))
        return s3
    }

    private fun playOneFullRoundToScoring(state: BankedMatch): BankedMatch {
        // player 1 turn
        val r1 = BankedMatchEngine.apply(state, Command.Roll(1))
        val f1 = BankedMatchEngine.apply(r1, Command.FinishTurn(1))
        // player 2 turn
        val r2 = BankedMatchEngine.apply(f1, Command.Roll(2))
        val f2 = BankedMatchEngine.apply(r2, Command.FinishTurn(2))
    ...
    fun `Start opens pot and collects ante from every wallet`() {
        ...
        assertEquals(90, started.wallets[1]!!.currentBalance)
        assertEquals(90, started.wallets[2]!!.currentBalance)
    }
    ...
    fun `NextRound settles previous pot and opens the next one when match continues`() {
        ...
        val afterNext = BankedMatchEngine.apply(scoring, Command.NextRound(byUserId = 10))
        ...
        assertEquals(90, afterNext.wallets[1]!!.currentBalance)
        assertEquals(90, afterNext.wallets[2]!!.currentBalance)
    ...
    fun `NextRound on last round settles and clears open pot when match finishes`() {
        ...
        val afterNext1 = BankedMatchEngine.apply(scoring1, Command.NextRound(byUserId = 10))

        // Round 2 -> SCORING
        val scoring2 = playOneFullRoundToScoring(afterNext1)
        // Settle R2 and finish match (no new pot)
        val finished = BankedMatchEngine.apply(scoring2, Command.NextRound(byUserId = 10))
        ...
        assertEquals(100, finished.wallets[1]!!.currentBalance)
    ...
    fun `Start finishes match and does not open pot when only one player can pay ante`() {
        ...
        assertEquals(100, started.wallets[1]!!.currentBalance)
        assertEquals(0, started.wallets[2]!!.currentBalance)
    }
