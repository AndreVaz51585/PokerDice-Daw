...
class LobbyServiceImpl(
    ...
    override fun getLobby(id: Int): Either<LobbyServiceError, Lobby> = trxManager.run {
    ...
    override fun joinLobby(lobbyId: Int, userId: Int): Either<LobbyServiceError, Boolean> = trxManager.run {
    ...
    override fun leaveLobby(lobbyId: Int, userId: Int) = trxManager.run {
        repoLobby.remove(lobbyId, userId)
    }


    override fun getLobbyHost(lobby: Lobby): Either<LobbyServiceError, User> = trxManager.run {
        val host = repoLobby.getLobbyHost(lobby) ?: return@run failure(LobbyServiceError.UserNotFound)
        return@run success(host)
    }


    override fun listPlayers(lobbyId: Int): List<User> = trxManager.run {
