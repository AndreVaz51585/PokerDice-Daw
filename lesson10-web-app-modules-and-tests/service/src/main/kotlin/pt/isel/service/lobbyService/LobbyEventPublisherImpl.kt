...
class LobbyEventPublisherImpl : LobbyEventPublisher {
    ...
    override fun publish(lobbyId: Int, event: LobbyEvent): Unit = lock.withLock {
        listeners[lobbyId]?.forEach { listener ->
            try { listener(event) } catch (_: Exception) {}
        }
    }

    override fun subscribe(lobbyId: Int, listener: (LobbyEvent) -> Unit): () -> Unit = lock.withLock {
        listeners.computeIfAbsent(lobbyId) { mutableListOf() }.add(listener)
        return { unsubscribe(lobbyId, listener) }
    }

    private fun unsubscribe(lobbyId: Int, listener: (LobbyEvent) -> Unit) = lock.withLock {
