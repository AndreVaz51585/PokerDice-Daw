...
class MatchEventPublisherImp : MatchEventPublisher {
    ...
    override fun publish(matchId: Int, event: MatchEvent): Unit = lock.withLock {
        listeners[matchId]?.forEach { listener ->
            try { listener(event) } catch (_: Exception) {}
        }
    }

    override fun subscribe(matchId: Int, listener: (MatchEvent) -> Unit): () -> Unit = lock.withLock {
        listeners.computeIfAbsent(matchId) { mutableListOf() }.add(listener)
        return { unsubscribe(matchId, listener) }
    }

    private fun unsubscribe(matchId: Int, listener: (MatchEvent) -> Unit) = lock.withLock {
