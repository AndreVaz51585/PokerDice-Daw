...
class RepositoryInvitationJdbi(
    ...
    override fun findByInvitationId(invitationId: InvitationId): Invitation? {
        val invitationQuery = handle.createQuery(
            """
        ...
        val createdByUser = repoUser.findById(invitationQuery.created_by)
            ?: throw IllegalStateException("Invitation creator with id ${invitationQuery.created_by} not found.")

        val usedByUser: User? = invitationQuery.used_by?.let {
        ...
        override fun findById(id: Int): Invitation? = throw UnsupportedOperationException("Find by integer ID is not supported for Invitation.")
        ...
        override fun deleteById(id: Int): Boolean = throw UnsupportedOperationException("Delete by integer ID is not supported for Invitation.")
