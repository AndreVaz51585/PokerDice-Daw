...
class RepositoryWalletJdbi(private val handle: Handle) : RepositoryWallet {
    ...
    override fun findById(id: Int): Wallet? {
        val wallet =
            handle.createQuery(WalletSql.SELECT_WALLET)
                .bind("user_id", id)
                .map { rs, _ ->
                    Wallet(
    ...
    override fun findAll(): List<Wallet> {
        val wallet =
            handle.createQuery(WalletSql.SELECT_ALL_WALLET)
                .map { rs, _ ->
                    Wallet(
                        userId = rs.getInt("user_id"),
                        currentBalance = rs.getInt("amount_coins"),
    ...
    override fun deleteById(id: Int): Boolean {
        val ret =
            handle.createUpdate(WalletSql.DELETE_BY_ID)
                .bind("user_id", id)
                .execute() > 0
        return ret
    }
